...
async function getLiveLkrRate(): Promise<number> {
  const now = Date.now();
  if (p2pRateCache && (now - p2pRateCache.fetchedAt < P2P_CACHE_TTL_MS)) {
    return p2pRateCache.rate;
  }
  try {
    ...
    if (prices.length > 0) {
      ...
      console.log(`[P2P Rate Helper] Fetched USDT/LKR rate: ${median}`);
      return median;
    }
    throw new Error("No valid prices found");
  } catch (err: any) {
    console.error("[P2P Rate Fallback] Failed to fetch Binance P2P rate in helper:", err.message);
    if (p2pRateCache) return p2pRateCache.rate;
    try {
      const setting = await storage.getSetting("CURRENCY_RATE_LKR");
      return setting?.value ? parseFloat(setting.value) : 300.0;
    } catch {
      return 300.0;
    }
  }
...
export async function registerRoutes(
  ...
  io.on("connection", (socket) => {
    socket.on("register-user", (data: { telegramId: string }) => {
      activeUserSockets.set(data.telegramId, socket.id);
      (socket as any).telegramId = data.telegramId;
      
      // Notify online admins
      io.to("admins").emit("user-online-status", {
    ...
    socket.on("get-online-users", () => {
      const onlineUsers: { telegramId: string; socketId: string }[] = [];
    ...
    socket.on("call-user", (data: { offer: any; callerName: string; callerId: string; toSocketId?: string; toTelegramId?: string }) => {
      ...
      if (targetSocketId) {
        // Direct targeted call (e.g. Admin calling User)
        ...
        // Broadcast call to admins (Customer calling support)
  ...
  app.post("/api/support/upload", 
    ...
    async (req: any, res: any) => {
      ...
      // Block dangerous executable extensions for security
  ...
  app.post("/api/mini/purchase", verifyMiniAppAuth, async (req, res) => {
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        const rate = currencySetting ? parseFloat(currencySetting.value) : (prodCurrency === 'LKR' ? await getLiveLkrRate() : 1.0);
        ...
        if (payCurrency !== 'USD') {
          ...
          const payRate = payCurrencySetting ? parseFloat(payCurrencySetting.value) : (payCurrency === 'LKR' ? await getLiveLkrRate() : 1.0);
  ...
  app.post("/api/mini/purchase-offer", verifyMiniAppAuth, async (req, res) => {
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        const rate = currencySetting ? parseFloat(currencySetting.value) : (productCurrency === 'LKR' ? await getLiveLkrRate() : 1.0);
...
app.post("/api/support/reply", isAuth, async (req, res) => {
  try {
    ...
    const finalMessage = message ? message.trim() : (attachmentType === 'image' ? '📷 Photo Attachment' : '📄 Attachment');
    ...
    if (!telegramId.startsWith("web_")) {
      if (!supportBot) {
        ...
        try {
          ...
          // Do not block - let the local mini-app web chat receive it
        }
      }
    }
...
app.get("/api/mini/p2p-rate", async (req, res) => {
  try {
    const rate = await getLiveLkrRate();
    res.json({ rate, cached: true, fetchedAt: p2pRateCache?.fetchedAt || Date.now() });
  } catch (err: any) {
...
`;
  ...
  app.get("/whatsapp", async (req, res) => {
    try {
      ...
      const url = (setting?.value || "https://wa.me/94760895782").trim();

      // Parse phone and text parameters
      let phone = "";
      let textParam = "";
