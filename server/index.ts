...
app.use((req, res, next) => {
  ...
  // Record traffic logs (exclude static assets, vite paths, and health checks)
  if (
    !path.includes('.') && 
    !path.startsWith('/vite') && 
    !path.startsWith('/api/health') && 
    !path.startsWith('/api/admin/traffic-stats')
  ) {
    ...
    import("./storage").then(({ storage }) => {
      storage.recordTraffic(cleanIp, path, userAgent);
